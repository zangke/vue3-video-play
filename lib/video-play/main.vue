...
<template>
  <div class="d-player-wrap" v-if="!props.src">
    <d-player-top
      :title="props.title"
      :showClose="!props.src"
      :noChannel="true"
    ...
    ></d-player-top>
    <div class="d-loading">
      <span> <i class="iconfont icon-replay f24 mr5"></i>无信号 </span>
    </div>
  </div>
  <div
    ref="refPlayerWrap"
  ...
  >
    ...
    <d-player-top
      ...
      :showClose="props.src"
      @channelChooseClick="channelChooseClick"
      @channelCloseClick="channelCloseClick"
    ></d-player-top>
    <!-- 状态栏 移动端不显示-->
    <div class="d-player-state" v-if="!isMobile">
      <transition name="d-scale-out">
        ...
        <div class="d-play-btn" v-show="state.playBtnState == 'play'">
          <d-icon icon="icon-play" :size="26"></d-icon>
        </div>
      </transition>
      <!-- 操作信息通知 -->
